...
if [ -f Hyprland-Dots.tar.gz ]; then
  ...
  latest_version=$(curl -s https://api.github.com/repos/PuppyAnimations/Hyprland-Dots/releases/latest | grep "tag_name" | cut -d '"' -f 4 | sed 's/v//')
  ...
  if [ "$existing_version" = "$latest_version" ]; then
    ...
    if [ "$upgrade_choice" = "y" ]; then
		...
		# Delete existing directories starting with PuppyAnimations-Hyprland-Dots
      find . -type d -name 'PuppyAnimations-Hyprland-Dots*' -exec rm -rf {} +
...
latest_tag=$(curl -s https://api.github.com/repos/PuppyAnimations/Hyprland-Dots/releases/latest | grep "tag_name" | cut -d '"' -f 4)
...
latest_tarball_url=$(curl -s https://api.github.com/repos/PuppyAnimations/Hyprland-Dots/releases/latest | grep "tarball_url" | cut -d '"' -f 4)
...
if curl -L "$latest_tarball_url" -o "$file_name"; then
  ...
  rm -rf PuppyAnimations-Hyprland-Dots
  ...
  mv "$extracted_directory" PuppyAnimations-Hyprland-Dots || exit 1

  cd "PuppyAnimations-Hyprland-Dots" || exit 1

  # Set execute permission for copy.sh and execute it
  chmod +x copy.sh
  ./copy.sh 

  echo -e "${OK} Latest Dotfiles release downloaded, extracted, and processed successfully. Check PuppyAnimations-Hyprland-Dots folder for more detailed install logs" 2>&1 | tee -a "../Install-Logs/install-$(date +'%d-%H%M%S')_dotfiles.log"
else
